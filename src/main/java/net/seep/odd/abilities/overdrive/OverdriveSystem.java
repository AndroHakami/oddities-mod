...
public final class OverdriveSystem {
    ...
    // In OverdriveSystem
    public static boolean isRelayActive(net.minecraft.server.network.ServerPlayerEntity p) {
        return data(p).relayActive;
    }
    public static boolean isInOverdrive(net.minecraft.server.network.ServerPlayerEntity p) {
        return data(p).mode == Mode.OVERDRIVE;
    }
    public static boolean hasFullMeter(net.minecraft.server.network.ServerPlayerEntity p) {
        // uses the same threshold the system uses internally
        return data(p).energy >= OVERDRIVE_MIN_EN;
    }
    ...
    public static void startPunch(ServerPlayerEntity p) {
        Data d = data(p);
        if (d.mode == Mode.NORMAL || d.energy < PUNCH_ENERGY_COST) { p.sendMessage(Text.literal("Need energy to charge punch."), true); return; }
        d.chargingPunch = true; d.chargeStartTick = p.getWorld().getTime();
        p.getWorld().playSound(null, p.getBlockPos(), SoundEvents.BLOCK_RESPAWN_ANCHOR_CHARGE, SoundCategory.PLAYERS, 0.5f, 1.8f);
    }

    public static void releasePunch(ServerPlayerEntity p) {
        Data d = data(p); if (!d.chargingPunch) return; d.chargingPunch = false;
        ...
        if (d.energy < PUNCH_ENERGY_COST) { p.sendMessage(Text.literal("Out of energy!"), true); return; }
        ...
        if (p.getWorld() instanceof ServerWorld sw) {
            for (Entity e : sw.getOtherEntities(p, area, ent -> ent instanceof LivingEntity && ent.isAlive())) {
                ...
                Vec3d push = le.getPos().subtract(p.getPos()).normalize().multiply(kb).add(0, 0.25, 0);
        ...
        p.getWorld().playSound(null, p.getBlockPos(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS, 0.5f, 1.6f);
    }
