...
public final class OuterMechLaserFx implements PostWorldRenderCallback {
    private static final Identifier POST_ID = new Identifier("odd", "shaders/post/outer_mech_extender_beam.json");
    ...
    private OuterMechLaserFx() {}

    public static void init() {
        if (inited) return;
        inited = true;
        PostWorldRenderCallback.EVENT.register(new OuterMechLaserFx());
    }
    ...
    public void onWorldRendered(Camera camera, float tickDelta, long limitTime) {
        ...
        for (OuterMechEntity mech : client.world.getEntitiesByClass(OuterMechEntity.class, range, e -> e.isAlive() && e.isExtenderBeamActive())) {
            var start = mech.getRailGunOrigin();
            var end = mech.getBeamEnd();
