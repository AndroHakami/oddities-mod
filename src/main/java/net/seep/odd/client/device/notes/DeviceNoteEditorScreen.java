...
import net.seep.odd.device.notes.NotesManager;
import net.seep.odd.device.notes.NotesNetworking;

@Environment(EnvType.CLIENT)
public final class DeviceNoteEditorScreen extends Screen {
    ...
    public DeviceNoteEditorScreen(Screen parent, UUID noteId, String content) {
        super(Text.literal(noteId == null ? "New Note" : "Edit Note"));
        this.parent = parent;
        this.noteId = noteId;
        this.existing = noteId != null;
        this.status = "";
    ...
    protected void init() {
        ...
        this.editor = new DeviceMultilineTextEditor(
                ...
                296,
                154,
                NotesManager.NOTE_MAX_LEN
        );
        this.editor.setPlaceholder(Text.literal("Write your note here...\nEnter = newline"));
        this.editor.setText(initialContent);
        this.editor.setFocused(true);

        this.addDrawableChild(ButtonWidget.builder(Text.literal("Save"), b -> saveNote())
                .dimensions(left + 12, top + 190, 92, 20)
                .build());

        if (existing) {
            this.addDrawableChild(ButtonWidget.builder(Text.literal("Delete"), b -> deleteNote())
    ...
    private void saveNote() {
        ...
        buf.writeBoolean(noteId != null);
        if (noteId != null) {
            buf.writeUuid(noteId);
        }
        buf.writeString(editor.getText(), NotesManager.NOTE_MAX_LEN);
        ClientPlayNetworking.send(NotesNetworking.C2S_SAVE_NOTE, buf);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(
                this.textRenderer,
                Text.literal("Chars: " + (editor == null ? 0 : editor.length()) + "/" + NotesManager.NOTE_MAX_LEN),
                left + 12,
                top + 184,
                0xFFE4EDFF
        );
        ...
        if (!status.isEmpty()) {
            context.drawCenteredTextWithShadow(this.textRenderer, Text.literal(status), this.width / 2, top + 184, 0xFFFFD0A0);
        }
