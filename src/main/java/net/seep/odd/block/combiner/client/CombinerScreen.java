...
public class CombinerScreen extends HandledScreen<CombinerScreenHandler> {
    ...
    // match server tolerance to feel consistent
    private static final int CLIENT_TOL = 4;
    ...
    protected void drawBackground(DrawContext ctx, float delta, int mouseX, int mouseY) {
        ...
        if (active && !prevActive) {
            qteStartWorldTime = mc.world.getTime() - handler.progress();
...
// when active ended:
        ...
        if (qteStartWorldTime < 0) qteStartWorldTime = mc.world.getTime() - handler.progress();
    ...
    private static int[] generateMarks(int diff, int dur, int seed) {
        ...
        while (i < diff && attempts++ < 500) {
            int m = 10 + r.nextInt(Math.max(1, dur - 20));
            boolean ok = true;
            for (int j = 0; j < i; j++) {
        ...
        while (i < diff) out[i++] = 10 + (i * (dur - 20) / Math.max(1, diff));
