...
void main() {
    ...
    for (int i = 0; i < STEPS; i++) {
        ...
        float startFade = smoothstep(0.00, 0.04, along01);
        float endFade = 1.0 - smoothstep(0.94, 1.00, along01);

        float shell = exp(-(dist * dist) / max(radius * radius * 0.55, 0.0001));
        float core = exp(-(dist * dist) / max(radius * radius * 0.065, 0.0001));

        float bands = 0.5 + 0.5 * sin(iTime * 12.0 - along01 * 22.0 + dist * 2.6);
        float rings = exp(-pow((dist - radius * 0.72) / max(radius * 0.20, 0.0001), 2.0)) * bands;

        bodySum += shell * startFade * endFade * dt * 2.2;
        coreSum += core * startFade * endFade * dt * 3.3;
        ringSum += rings * startFade * endFade * dt * 1.2;

        t += dt;
    }

    bodySum = clamp(bodySum * master, 0.0, 2.0);
    coreSum = clamp(coreSum * master, 0.0, 2.4);
    ringSum = clamp(ringSum * master, 0.0, 1.4);

    vec3 greenA = vec3(0.10, 1.00, 0.26);
    vec3 greenB = vec3(0.40, 1.00, 0.48);
    vec3 lime   = vec3(0.86, 1.00, 0.62);
    vec3 white  = vec3(0.96, 1.00, 0.94);

    vec3 glow = vec3(0.0);
    glow += greenA * bodySum * 0.95;
    glow += greenB * bodySum * 0.72;
    glow += lime   * ringSum * 0.65;
    glow += white  * coreSum * 0.82;
